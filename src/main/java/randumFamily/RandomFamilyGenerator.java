package randumFamily;

public class RandomFamilyGenerator {
  private final RandomUserApi randomUserApi;

  public RandomFamilyGenerator(RandomUserApi randomUserApi) {
    this.randomUserApi = randomUserApi;
  }

  public FamilyEntity createFamily(int sons, int daughters) {
    //TODO: Using the Random User API and the RxJava facilities, construct a new instance of RandomFamilyGenerator.
    //Keep in mind to follow the anglo-saxon tradition related to the last name of the family members.
    return null;
  }
}
